...
let isGameRunning = false; // Flag to stop physics until start
...
// Don't start loop immediately, wait for init
animate(); 
...
function setupStartScreen() {
    ...
    startBtn.addEventListener('click', () => {
        ...
        // 2. Try to keep screen awake (Wake Lock API)
...
function setupLights() {
    ...
    dirLight.shadow.mapSize.set(1024, 1024); // Lowered slightly for mobile perfromance
    ...
    for (let x = -size; x <= size; x++) {
        for (let z = -size; z <= size; z++) {
            ...
            if (x > -14 && x < 14 && z > -14 && z < 14) {
                if (Math.random() > 0.96) buildTree(x, height + 1, z);
            }
        }
    }
...
function buildHouse(x, y, z) {
    for(let i=0; i<5; i++) for(let j=0; j<5; j++) createBlock(x+i, y, z+j, matStone);
    for(let h=1; h<4; h++) {
        for(let i=0; i<5; i++) {
            for(let j=0; j<5; j++) {
                if(i===0 || i===4 || j===0 || j===4) {
    ...
    for(let layer=0; layer<3; layer++) {
        for(let i=0+layer; i<5-layer; i++) for(let j=0+layer; j<5-layer; j++) createBlock(x+i, y+4+layer, z+j, matWood);
    }
}
...
function createNPC(x, y, z) {
    ...
    npcs.push({ mesh: group, velocity: new THREE.Vector3(), timer: 0, dir: new THREE.Vector3(1,0,0) });
}
...
function animate() {
    ...
    if(!isGameRunning) {
        // Just render the background scene while waiting at menu
    ...
    if(playerGroup.position.y < -20) { playerGroup.position.set(0, 10, 0); velocityY = 0; }
    ...
    npcs.forEach(npc => {
        ...
        if(npcHits.length > 0) {
            ...
            if(npc.mesh.position.y < groundY) npc.mesh.position.y = groundY;
            else if (npc.mesh.position.y > groundY + 0.1) npc.mesh.position.y -= GRAVITY * delta;
